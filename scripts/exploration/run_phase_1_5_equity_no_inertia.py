"""Phase 1.5 ω-sweep: equity fitness, NO inertia (λ=0.0).

Companion to run_phase_1_5.py and run_phase_1_5_bilateral_no_inertia.py. Same
grid, same parallelism, same metric collection — only the CONFIG_BASE
fitness_basis and the output CSV filename differ. This is the equity-baseline
cell of the (basis × inertia) comparison.

Grid: 6 omegas × 5 etas × 3 seeds = 90 sims.

Run from the econo-ML-TBTF-clean directory:
    python run_phase_1_5_equity_no_inertia.py
"""
import os
import sys
import math
...
CSV_PATH = os.path.join(SAVE_DIR, 'phase1_5_omega_sweep_equity_l0.csv')


# ── config ───────────────────────────────────────────────────────────────────
# Equity fitness, λ=0 (no inertia). omega and eta swept; seed varied.
CONFIG_BASE = dict(
    N=50, T=1000,
    mu=0.7,
    rho=0.4,
...
def main():
    ...
    print(f"Phase 1.5 omega sweep (equity λ=0.0):")
