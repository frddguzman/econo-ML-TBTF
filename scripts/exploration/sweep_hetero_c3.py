"""Group 1: hetero at C3 (mu=0.6, omega=0.7) across the 3 gamma baselines.

6 cells: gamma in {0.05, 0.10, 0.12} x cv in {0.5, 1.0}
Each cell: eta in {0, 0.1} social, 5 seeds (26462-26466) = 10 sims.
Total: 60 sims, ~10 min.

Reference (no hetero) values from earlier multi-seed tests:
  gamma=0.05: max_ten 12.8 +/- 2.95, claim 3 delta -379
  gamma=0.10: max_ten 10.0 +/- 3.46, claim 3 delta -494
  gamma=0.12: max_ten 11.2 +/- 4.44, claim 3 delta -785

Question: does hetero on top of these baselines push hub stats further while
preserving claim 3?
"""
import os
...
def main():
    ...
    print(f'Group 1: hetero at C3 ({len(jobs)} sims, {WORKERS} workers)')
    rows = []
    ...
    out_csv = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sweep_hetero_c3_raw.csv')
    ...
    print()
    print('=== 5-seed aggregates per (gamma, cv) cell at C3 + hetero (raw-id turnovers) ===')
    print(f'{"gamma":>6} | {"cv":>4} | {"bk_eta0":>9} | {"bk_eta01":>9} | {"delta":>7} | {"claim3":>7} | '
    ...
    for g in GAMMAS:
        for cv in CVS:
            ...
            print(f'{g:>6.2f} | {cv:>4.1f} | {bk0:>9.0f} | {bk1:>9.0f} | {delta:>+7.0f} | {verdict:>7} | '
    ...
    print()
    print('Reference at C3 (no hetero), single-seed dashboard view: max_ten 14, turnovers 633.')
    print('NOTE: turnovers are now RAW-ID (matches dashboard). Earlier composite-key reports needed +1-3 on max_ten.')
