"""Brini-style Hub diagnostics for w58 cv=0 (mu=0.7, omega=0.58, gamma=0.10) at social regime.
Same style as plot_w55_truncated_brini.py for apples-to-apples comparison.

etas: {0.0, 0.1, 0.3, 0.8}
seeds: {26462, 26463, 26464, 26465, 26466}

Output: 4 PNGs w58_brini_e{XX}.png (e00 -> 0.0, e01 -> 0.1, e03 -> 0.3, e08 -> 0.8)
"""
import os
...
def plot_one_eta(eta, results_for_eta, out_path):
    ...
    fig.suptitle(f'Brini-style hub diagnostics — w58 cv=0 (mu={MU}, omega={OMEGA}, gamma={GAMMA}) + social + eta={eta}',
                 fontsize=12, y=1.005)
    fig.tight_layout()
...
def main():
    ...
    n = len(jobs)
    print(f'Running {n} sims at w58 cv=0 + social...', flush=True)
    results = []
    ...
    for eta in ETAS:
        ...
        out_path = os.path.join(PROJ, f'w58_brini_e{eta_int:02d}.png')
