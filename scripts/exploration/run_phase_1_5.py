"""Phase 1.5: omega sweet-spot sweep at bilateral λ=0.5.

Goal: locate an ω where lender plateaus emerge at ω=0.55-strength dynamics
AND η* is still detectable at ω=0.50-strength fiscal pressure. Bilateral λ=0.5
is fixed because it was the η*-preserving spec at ω=0.50 in phase1_grid.csv —
if a sweet spot exists for any spec, this is the most likely candidate.

Grid: 6 omegas × 5 etas × 3 seeds = 90 sims.

Parallelism: ProcessPoolExecutor with workers = cpu_count - 1. Each
(omega, eta, seed) is a task. CSV is written by a single result-collector
loop in the main process as futures complete (no lock needed — only one
writer). Worker function `run_one` is at module level so it pickles cleanly.

Run from the econo-ML-TBTF-clean directory:
    python run_phase_1_5.py
"""
import os
import sys
import math
...
# ── output paths (mirror run_phase1.py) ──────────────────────────────────────
SAVE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        '..', 'Simulations')
os.makedirs(SAVE_DIR, exist_ok=True)
CSV_PATH = os.path.join(SAVE_DIR, 'phase1_5_omega_sweep.csv')


# ── config ───────────────────────────────────────────────────────────────────
# Bilateral λ=0.5 fixed for the entire grid. omega and eta swept; seed varied.
# equity_heterogeneity left at Config default (False) for parity with the
# byte-identity baseline used in phase1_grid.csv.
CONFIG_BASE = dict(
    N=50, T=1000,
    mu=0.7,
    rho=0.4,
...
# Column order matches phase1_grid.csv (seed, basis, inertia, eta, …) with
# omega inserted between the parameter group and the eta dimension. basis and
# inertia are kept (constant per row) so this CSV concats cleanly with
# phase1_grid.csv on the shared column set.
CSV_COLUMNS = [
...
# ── helpers (copied from run_phase1.py so workers don't need cross-imports) ──
...
def compute_tenure(best_lender, best_lender_gen, T):
    """RLE on the composite (id, generation) key."""
...
def run_one(omega, eta, seed):
    """One simulation. Identical metric collection and clamp instrumentation
    to run_phase1.run_one but parameterized by (omega, eta, seed); basis and
    inertia come from CONFIG_BASE."""
    ...
    # Per-simulation clamp counters as instance attributes (no module globals,
    # safe across worker re-use within a process)
    m._clamp_total = 0
    m._clamp_hits = 0
...
def main():
    ...
    print(f"Phase 1.5 omega sweep (bilateral λ=0.5):")
