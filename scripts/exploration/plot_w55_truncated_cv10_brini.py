"""Brini-style Hub diagnostics for w55 + truncated lognormal cv=1.0 (cap=2.5, floor=0.5).
Same style as plot_w55_truncated_brini.py and plot_w58_brini.py.

Note: cv=1.0 (not 0.7) — this is the cell with the HIGHEST avg_ten=2.73 in the
540-sim Approach B sweep, claimed as the "best avg_ten Approach B cell".

etas: {0.0, 0.1, 0.3, 0.8} (matches w58 reference)
seeds: {26462, 26463, 26464, 26465, 26466}

Output: 4 PNGs w55_trunc_cv10_brini_e{XX}.png
"""
import os
...
def main():
    ...
    for eta in ETAS:
        ...
        out_path = os.path.join(PROJ, f'w55_trunc_cv10_brini_e{eta_int:02d}.png')
