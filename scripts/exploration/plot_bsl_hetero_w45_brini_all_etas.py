"""Generate Brini-style figures for ALL etas at bsl + equity + hetero(cv=0.7) + omega=0.45 + gamma=0.10.

Same format as plot_c3_bilateral_brini_all_etas.py for direct comparison.
Each figure: 5 seed rows × 2 time-cols.

Output: 10 PNGs named bsl_hetero_w45_brini_e{XX}.png (e00 -> eta=0.0, ..., e09 -> eta=0.9)
"""
import os
...
def plot_one_eta(eta, results_for_eta, out_path):
    ...
    fig.suptitle(f'Brini-style hub diagnostics — bsl + equity + hetero(cv=0.7) + omega={OMEGA} + gamma=0.10 + eta={eta}',
                 fontsize=12, y=1.005)
    fig.tight_layout()
...
def main():
    ...
    n = len(jobs)
    print(f'Running {n} sims at bsl + equity + hetero(0.7) + omega={OMEGA}...', flush=True)
    results = []
    ...
    for eta in ETAS:
        ...
        out_path = os.path.join(PROJ, f'bsl_hetero_w45_brini_e{eta_int:02d}.png')
