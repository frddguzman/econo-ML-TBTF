"""D1 multi-regime + multi-tau sweep at w58 cv=1.0 + reintroduce_with_median=False.

Completes the 3-fiscal-regime story for the heterogeneity exploration. Existing data:
  - socialized_tax: full eta sweep complete (sweep_d1_full_eta_raw.csv)

This sweep adds:
  - no_tax (1 tau value, irrelevant)
  - resolution_fund at tau=1e-4 (default, dashboard 'w58' equivalent)
  - resolution_fund at tau=1e-5 (canonical fund-regime tau, dashboard 'w58_t5' equivalent)
  - resolution_fund at tau=1e-6 (dashboard 'w58_t6' equivalent)

Spec:
  - cand: w58 (mu=0.7, omega=0.58, gamma=0.10), cv=1.0, init_replace
  - 15 etas: {0.0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50, 0.60, 0.70, 0.75, 0.80, 0.85, 0.90}
  - 5 seeds: {26462-26466}
  - 4 (regime, tau) combinations: nt, rf@1e-4, rf@1e-5, rf@1e-6
  - Total: 4 x 15 x 5 = 300 sims, ~25 min wall

Output: sweep_d1_multi_regime_raw.csv with all (regime, tau) tagged.
"""
import os
...
# (regime, tau_fund_for_log)
# tau_fund only matters under resolution_fund regime; under no_tax it's a no-op
REGIMES = [
    ('no_tax',          1e-4),  # tau is no-op here
    ('resolution_fund', 1e-4),  # default tau
    ('resolution_fund', 1e-5),  # canonical commit tau
    ('resolution_fund', 1e-6),
]

OUT_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sweep_d1_multi_regime_raw.csv')
...
def run_one(args):
    ...
    return {
        'cand': 'w58_h1', 'mu': 0.70, 'omega': 0.58, 'gamma': 0.10, 'cv': 1.0,
        'reintr_median': False,
...
def main():
    ...
    n = len(jobs)
    print(f'D1 multi-regime sweep: w58 cv=1.0 + reintroduce_with_median=False', flush=True)
    print(f'  4 (regime, tau): no_tax + resolution_fund at tau in {{1e-4, 1e-5, 1e-6}}', flush=True)
    print(f'  15 etas, 5 seeds, total {n} sims, {WORKERS} workers', flush=True)
