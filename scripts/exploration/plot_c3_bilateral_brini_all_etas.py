"""Generate Brini-style figures for ALL etas (0.0 to 0.9 step 0.1) at C3 + tier_init
+ bilateral + mult=1.75. Each figure has 5 seed rows x 2 time-cols.

Output: 10 PNGs named c3_bilateral_brini_e{XX}.png (e00 -> eta=0.0, ..., e09 -> eta=0.9)
"""
import os
...
def plot_one_eta(eta, results_for_eta, out_path):
    """5 seeds x 2 cols (t=[0,500], [500,1000]) Brini-style figure for one eta."""
    ...
    fig.suptitle(f'Brini-style hub diagnostics — C3 + tier_init + bilateral + mult={MULT} + eta={eta}',
                 fontsize=12, y=1.005)
    fig.tight_layout()
...
def main():
    ...
    n = len(jobs)
    print(f'Running {n} sims (5 seeds × {len(ETAS)} etas) at C3 + tier_init + bilateral + mult={MULT}...', flush=True)
    results = []
    ...
    print(f'All sims complete. Generating {len(ETAS)} Brini PNGs...', flush=True)
    for eta in ETAS:
        rs = [r for r in results if abs(r['eta'] - eta) < 1e-9]
        eta_int = int(round(eta * 10))
        out_path = os.path.join(PROJ, f'c3_bilateral_brini_e{eta_int:02d}.png')
    ...
    # Also dump a CSV for future analysis
    csv_path = os.path.join(PROJ, 'c3_bilateral_brini_summary.csv')
    with open(csv_path, 'w', encoding='utf-8') as f:
        f.write('eta,seed,T,max_ten,avg_ten,turnovers,avg_cli,total_bk\n')
        for r in sorted(results, key=lambda x: (x['eta'], x['seed'])):
            mt, av, to = compute_tenure_stats(r['hub_id'])
            avgc = float(np.nanmean(r['hub_cli'])) if not np.all(np.isnan(r['hub_cli'])) else 0
            f.write(f'{r["eta"]},{r["seed"]},{r["T"]},{mt},{av:.2f},{to},{avgc:.2f},{r["total_bk"]}\n')
