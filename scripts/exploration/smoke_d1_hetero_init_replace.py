"""D1 smoke: hetero + reintroduce_with_median=False.

Hypothesis: under hetero, each bank has its own initial E from lognormal draw.
If failed banks come back at SAME-distribution lognormal draw (instead of survivor median),
the median doesn't drift toward small. Champion that fails → next replacement could
be initially-rich → hub competitiveness without lock-in.

Spec:
  - cands: bsl (mu=0.7, omega=0.50), w58 (mu=0.7, omega=0.58)
  - cv: {0.7, 1.0}
  - etas: {0.0, 0.1, 0.3, 0.85}
  - seeds: {26462-26466}
  - reintroduce_with_median: False (the D1 axis)
  - regime: socialized_tax
  - Total: 2 cands x 2 cv x 4 etas x 5 seeds = 80 sims, ~2 min wall

Output: smoke_d1_raw.csv (full schema for comparison vs Approach B).
"""
import os
...
OUT_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'smoke_d1_raw.csv')
...
def main():
    ...
    n = len(jobs)
    print(f'D1 smoke: hetero + reintroduce_with_median=False', flush=True)
