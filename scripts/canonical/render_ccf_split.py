"""Split-figure CCF render for §3.3.2 — produces two PNGs matching the two
analytical beats:

  3_3_2_ccf_chain.png    — pairs 0, 2, 3 (shock→contagion, shock→survivors,
                           survivors→contagion) = the shock-to-contagion-chain figure
  3_3_2_ccf_fiscal.png   — pairs 1, 4 (contagion→fiscal, bailout→fiscal)
                         = the fiscal-row architecture figure

Same data extraction + CCF logic as ccf_w58_full_regimes.py; just renders two
narrower figures instead of one tall 5×3 grid.

Single-seed SEED=26474, canonical w58_t5 (τ=1e-5), all 3 regimes at η=0.10.
"""
import os, sys, json
...
CANDS = ['w58_t5']
CAND_LABELS = {'w58_t5': ''}  # blank cand label — regime label alone is enough
CELLS = [(c, r, f'{c}_{r}_e01') for c in CANDS for r in REGIMES]

CHANNEL_PAIRS = [
    ('bankruptcies_shock', 'bankruptcies_contagion'),       # 0
    ('bankruptcies_contagion', 'bankruptcies_fiscal'),      # 1
    ('bankruptcies_shock', 'fire_sale_survivors'),          # 2
    ('fire_sale_survivors', 'bankruptcies_contagion'),      # 3
    ('bailout_bill', 'bankruptcies_fiscal'),                # 4
]
PAIR_LABELS = [
    'shock $\\to$ contagion',
    'contagion $\\to$ fiscal',
...
# Subsets for the two split figures
CHAIN_PAIRS = [0, 2, 3]   # shock→contagion, shock→survivors, survivors→contagion
FISCAL_PAIRS = [1, 4]     # contagion→fiscal, bailout→fiscal
...
def main():
    ...
    # Compute CCFs for ALL pairs (we render subsets)
    results = {}
    ...
    # Render the chain figure (pairs 0, 2, 3)
    chain_path = os.path.join(OUT_DIR, '3_3_2_ccf_chain.png')
    chain_title = (f'Channel CCFs: shock-to-contagion chain at $\\eta=0.10$ '
    ...
    # Render the fiscal figure (pairs 1, 4)
    fiscal_path = os.path.join(OUT_DIR, '3_3_2_ccf_fiscal.png')
    fiscal_title = (f'Channel CCFs: fiscal-row architecture at $\\eta=0.10$ '
                    f'(single-seed SEED${{}}=26474$)\n'
                    f'95% noise band $\\pm{ci:.3f}$')
