"""2D alpha x rho cross-sweep at canonical w58 cv=0.

Goal: identify whether a different (alpha, rho) combination produces better dynamics
while preserving the canonical claims (1, 2, 3).

alpha = collateral haircut coefficient in eq.6 numerator (γ·E_i + p_j·(1-b_j)·α·A_j)
rho   = fire-sale recovery price (Section claim_2)

Both interact: alpha controls how much collateral lenders recover in no-bailout state;
rho controls how much equity is destroyed when a bank fire-sales assets.

Spec:
  - cand: w58 cv=0 (canonical)
  - alpha: {0.02, 0.05, 0.10, 0.20}        [default 0.05]
  - rho:   {0.30, 0.40, 0.50, 0.70}        [default 0.40]
  - eta:   {0.0, 0.10, 0.50, 0.85}         [4 etas - verify claim 3 U-shape]
  - 3 regimes (nt, st, rf at default tau=1e-4)
  - 5 seeds
  - Total: 4 x 4 x 4 x 3 x 5 = 960 sims, ~80 min wall

Output: sweep_w58_alpha_rho_raw.csv (slice ρ=0.4 to get α-only sweep view).
"""
import os
...
RHOS = [0.30, 0.40, 0.50, 0.70]
ETAS = [0.0, 0.10, 0.50, 0.85]   # 4 etas: baseline + claim3 optimum + mid + fiscal-drag
REGIMES = ['none', 'socialized_tax', 'resolution_fund']
WORKERS = 6

OUT_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sweep_w58_alpha_rho_raw.csv')

KEYS = ['cand', 'mu', 'omega', 'gamma', 'alpha_collateral', 'rho',
...
def run_one(args):
    alpha, rho, eta, regime, seed = args
    ...
    return {
        ...
        'alpha_collateral': alpha, 'rho': rho,
...
def main():
    jobs = [(a, r, e, reg, s) for a in ALPHAS for r in RHOS for e in ETAS
            for reg in REGIMES for s in SEEDS]
    n = len(jobs)
    print(f'2D alpha x rho sweep at w58 cv=0', flush=True)
