"""§3.1.2 — Topology snapshots at η=0.85 (above selectivity flip η†=5/6).

Mirrors render_topology_snapshots.py at η=0.85.
Reads ../Simulations/topology_w58_{nt,st,t5_rf}_e085.json + dashboard CSVs at e085 (if available).
"""
import os, sys, json
...
setup_mpl()
OUT = asset_path('ch3_1_topology', '3_1_2_topology_snapshots_e085.png')

REGIMES = [
    ('nt',    'No tax',                      '../Simulations/topology_w58_nt_e085.json'),
    ('st',    'Ex-post tax',                 '../Simulations/topology_w58_st_e085.json'),
    ('t5_rf', r'Ex-ante $\tau{=}10^{-5}$',   '../Simulations/topology_w58_t5_rf_e085.json'),
]


def get_peak_client_t(regime_tag):
    """Find period with highest best_lender_clients in dashboard CSV (η=0.85 if available)."""
    if regime_tag == 't5_rf':
        path = '../Simulations/dash_w58_t5_rf_e085.csv'
    else:
        path = f'../Simulations/dash_w58_{regime_tag}_e085.csv'
    if not os.path.exists(path):
        return 999  # fallback if no dash CSV at e085
...
def main():
    ...
    fig.suptitle(r'Topology snapshots at canonical w58 cv=0, $\eta=0.85$ '
                 r'(above selectivity flip $\eta^\dagger=5/6$) — spring layout, hub in blue, '
