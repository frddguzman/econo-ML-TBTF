"""alpha x rho cross-sweep, FUND REGIME ONLY at canonical tau=1e-5.

Why a separate sweep: existing sweep_w58_alpha_rho_raw.csv has fund-regime cells
at default tau=1e-4. Canonical commit is tau=1e-5; need fund cells re-run for
canonical-consistency. nt and st are tau-invariant (no fund), already correct.

Spec:
  - cand: w58 cv=0 (canonical)
  - alpha: {0.02, 0.05, 0.10, 0.20}
  - rho:   {0.30, 0.40, 0.50, 0.70}
  - eta:   {0.0, 0.10, 0.50, 0.85}
  - regime: resolution_fund only, fund_levy_rate=1e-5
  - 5 seeds
  - Total: 4 x 4 x 4 x 5 = 320 sims, ~27 min wall

Output: sweep_w58_alpha_rho_fund_t5_raw.csv
"""
import os
...
OUT_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sweep_w58_alpha_rho_fund_t5_raw.csv')
...
def main():
    jobs = [(a, r, e, s) for a in ALPHAS for r in RHOS for e in ETAS for s in SEEDS]
    n = len(jobs)
    print(f'alpha x rho cross-sweep at w58 cv=0, fund regime, tau=1e-5 (canonical)', flush=True)
